...
export class UserController {
  ...
  // 注册接口
  @Post('register')
  register(@Body() registerPar: User, @Session() session) {
    const { authCode } = registerPar
    ...
    for(let key in registerPar) {
      if(!this.regRules[key].test(registerPar[key])) {
        return {
          code: 400,
          msg: "参数错误"
        }
    ...
    // 验证用户名或邮箱或手机号是否已经注册
    this.userService.checkExisted(registerPar.userName, registerPar.email, registerPar.phone)
    
    let res = this.userService.register(registerPar)
    return {
      code: res ? 200 : 400,
      msg: "注册" + (res ? "成功" : "失败")
    }
