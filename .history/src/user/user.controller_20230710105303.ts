...
export class UserController {
  ...
  // 注册接口
  @Post('register')
  register(@Body() registerPars: User, @Session() session) {
    const { authCode } = registerPars
    ...
    for(let key in registerPars) {
      if(!this.regRules[key].test(registerPars[key])) {
        return {
          code: 400,
          msg: "参数错误"
        }
    ...
    // 验证用户名或邮箱或手机号是否已经注册
    this.userService.checkExisted(registerPars.userName, registerPars.email, registerPars.phone)
    
    let res = this.userService.register(registerPars)
    return {
      code: res ? 200 : 400,
      msg: "注册" + (res ? "成功" : "失败")
    }
