...
export class UserController {
  ...
  async getCode(@Req() req: Request, @Res() res: Response) {
    ...
    req.session.code = request.session.visits ? request.session.visits: svgCaptcha.text; // 使用session保存验证，用于登陆时验证
  ...
  // 注册接口
  @Post('register')
  register(@Body() registerParams: CreateUserDto, @Req() req: Request) {
    console.log(req.session);

    const { authCode } = registerParams

    if(authCode?.toUpperCase() !== req.session.code?.toUpperCase()){
      return {
        code: 400,
        msg: "验证码错误"
      }
